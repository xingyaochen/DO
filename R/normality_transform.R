...
ggplot(data, aes(ACR1, col=Sex, group=Sex))+
...
for(i in 7:150){
  ...
  qqnorm(as.numeric(data.rz[,i]), main=paste("Normal Q-Q Plot","RankZ",
                               names(data.rz)[i]))
  qqline(as.numeric(data.rz[,i]), col=2)
  data.log[,i][!is.finite(data.log[,i])]=NA
  qqnorm(as.numeric(data.log[,i]), main=paste("Normal Q-Q Plot","Log",
                                names(data.log)[i]))
